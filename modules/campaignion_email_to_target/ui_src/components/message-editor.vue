<template>
  <fieldset class="message-editor">
    ...
    <template v-if="type == 'exclusion'">
      <div class="form-group">
        <label for="message-body-{{ _uid }}">Message shown if no target is available <a href="#" @click.prevent="toggleHelpText('body')" class="show-help-text"><span>?</span></a></label>
        <textarea rows="6" v-model="message.body" data-token-insertable class="form-control" id="message-body-{{ _uid }}"></textarea>
        <small v-if="helpText['body']" class="text-muted">This message is shown if this exclusion comes into effect and no targets are found for a supporter.</small>
      </div>
    </template>
  </fieldset>
</template>
